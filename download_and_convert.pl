...
#Set up options
my %opt=();
GetOptions (\%opt,'source=s','type=s','auth_key=s','force','help','man') or pod2usage(2);
...
while(my $sample_name=<>){
    chomp($sample_name);
    my $dir =catfile($download_dir,$opt{'type'},$opt{'source'});
    my $file=catfile($dir,$sample_name);
    make_path($dir);

    my $biom_file=$file.'.biom';
    
    if((!-e $biom_file) || $opt{'force'}){
	my $curl_cmd= $base_cmd . "\"http://api.metagenomics.anl.gov/abundanceprofile/mgm$sample_name\?type=$opt{'type'}&source=$opt{'source'}\" > $biom_file";
	print $curl_cmd,"\n";
	system($curl_cmd);
    }else{
...
For type 'function': KO, NOG, COG, Subsystems

For type 'organism' or 'feature': M5NR, SwissProt, GenBank, IMG, SEED, TrEMBL, RefSeq, PATRIC, KEGG, M5RNA, RDP, Greengenes, LSU, SSU

Working: KO, Greengenes, RefSeq, KEGG, RDP, GenBank (maybe)

Not Working: NOG, COG, Subsystems, M5NR, SwissProt, IMG, SEED, TrEMBL, PATRIC, M5RNA, LSU, SSU

(See MGRAST API documentation: http://api.metagenomics.anl.gov/api.html#abundanceprofile)
...
B<download_and_convert.pl> - Downloads each sample in BIOM format and converts to a nicer tab delimited format. Requires that BIOM be installed. Should work for either taxonomy or function. However, MG-RAST is currently not formatting their BIOM files correctly so many options will not work. 
